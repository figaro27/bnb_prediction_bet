...
contract BnbPricePredictionBet is Ownable, Pausable {
    ...
    function genesisLockRound() external onlyOperator whenNotPaused {
        ...
        require(
            block.timestamp <= rounds[currentEpoch].lockTime.add(bufferTime),
            "Can only lock round within bufferTime--"
        );
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        BetInfo memory betInfo = ledger[epoch][user];
        Round memory round = rounds[epoch];
        return !round.oracleCalled && block.timestamp > round.endTime.add(bufferTime) && betInfo.amount != 0;
    }
    ...
    function _safeLockRound(uint256 epoch, int256 price) internal {
        ...
        require(block.timestamp <= rounds[epoch].lockTime.add(bufferTime), "Can only lock round within bufferTime");
    ...
    function _safeEndRound(uint256 epoch, int256 price) internal {
        ...
        require(block.timestamp <= rounds[epoch].endTime.add(bufferTime), "Can only end round within bufferTime");
